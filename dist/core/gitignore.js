...
function isManagedEntry(entry) {
    return PRODIFY_IGNORES.includes(normalizeIgnoreEntry(entry));
}
function isBlanketProdifyRule(entry) {
    return BLANKET_PRODIFY_PATTERNS.has(normalizeIgnoreEntry(entry));
}
async function detectTrackedRuntimeFiles(repoRoot) {
...
export async function syncProdifyGitignore(repoRoot) {
    ...
    const existingNormalizedEntries = new Set(filteredLines
        .map((line) => line.trim())
