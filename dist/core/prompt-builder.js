...
export function buildRuntimeCommandReference(options = {}) {
    ...
    const lines = [
        ...
        '- Iteration-scoped execution input: `.prodify/runtime/current-iteration.json` plus delta files under `.prodify/runtime/`.',
        ...
        '- `$prodify-execute`: run one stage from the current iteration/delta runtime inputs, write stage artifacts, validate them against compiled contracts, then pause in interactive mode.',
        ...
        '- `$prodify-resume`: continue from `.prodify/state.json` and the current iteration/delta runtime inputs after a pause, interruption, or validation checkpoint.',
        '',
        'Stage order:',
