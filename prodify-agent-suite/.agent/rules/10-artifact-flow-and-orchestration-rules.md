...
## Artifact Location
All task artifacts live in `.agent/artifacts/`.

## Handoff Rules
- Every task must read its declared inputs from `.agent/artifacts/`.
...
- After completing a task, validate the output structure against the matching template in `.agent/templates/`.
...
## Workflow State
Use `.agent/artifacts/run_state.json` as the source of truth for:
- current task
- last completed task
- next task
...
Use `.agent/artifacts/task_log.json` to append execution history.
