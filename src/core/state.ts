...
function createEmptyEnforcementLoop(): EnforcementLoopState {
  return {
    stage: null,
...
function normalizeValidation(value: unknown): StageValidationResult | null {
  ...
  return {
    ...
    ...(typeof record.score_delta === 'object' && record.score_delta !== null ? { score_delta: record.score_delta as StageValidationResult['score_delta'] } : {}),
...
function normalizeEnforcementLoop(value: unknown): EnforcementLoopState {
  const record = asRecord(value);
  ...
  return {
    ...
    unmet_requirement_rules: Array.isArray(record.unmet_requirement_rules)
      ? record.unmet_requirement_rules.filter((entry): entry is string => typeof entry === 'string')
      : [],
    last_diff_result: typeof record.last_diff_result === 'object' && record.last_diff_result !== null
      ? record.last_diff_result as EnforcementLoopState['last_diff_result']
      : null,
    last_score_delta: typeof record.last_score_delta === 'object' && record.last_score_delta !== null
      ? record.last_score_delta as EnforcementLoopState['last_score_delta']
      : null,
    last_validation_delta: typeof record.last_validation_delta === 'object' && record.last_validation_delta !== null
      ? {
        failed_checks: Array.isArray(asRecord(record.last_validation_delta).failed_checks)
          ? (asRecord(record.last_validation_delta).failed_checks as unknown[]).filter((entry): entry is string => typeof entry === 'string')
          : [],
        threshold_gaps: Array.isArray(asRecord(record.last_validation_delta).threshold_gaps)
          ? (asRecord(record.last_validation_delta).threshold_gaps as unknown[]).filter((entry): entry is string => typeof entry === 'string')
          : [],
        missing_evidence: Array.isArray(asRecord(record.last_validation_delta).missing_evidence)
          ? (asRecord(record.last_validation_delta).missing_evidence as unknown[]).filter((entry): entry is string => typeof entry === 'string')
          : [],
        hotspot_targets: Array.isArray(asRecord(record.last_validation_delta).hotspot_targets)
          ? (asRecord(record.last_validation_delta).hotspot_targets as unknown[]).filter((entry): entry is string => typeof entry === 'string')
          : []
      }
    ...
    last_refactor_impact_report: typeof record.last_refactor_impact_report === 'object' && record.last_refactor_impact_report !== null
      ? record.last_refactor_impact_report as EnforcementLoopState['last_refactor_impact_report']
      : null,
