...
import { syncManagedTargets } from './sync.js';
...
export async function updateProdifySetup(repoRoot) {
  ...
  const targetResults = await syncManagedTargets(repoRoot, {
    createMissing: true
  });
  ...
  return {
    ...
    updatedTargets: targetResults.filter((result) => result.status === 'updated' || result.status === 'unchanged'),
    skippedTargets: targetResults.filter((result) => result.status !== 'updated' && result.status !== 'unchanged')
  };
}
